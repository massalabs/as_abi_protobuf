...
export function main(_args: ArrayBuffer): ArrayBuffer {
  ...
  env.generate_str_event("test check_native_amount");
  const is_valid = env.check_native_amount(amount1);
  env.generate_str_event("amount1 is valid: " + is_valid.toString());

  env.generate_str_event("test native_amount_to_string");
  ...
  env.generate_str_event("test native_amount_from_string");
  const amount3 = env.native_amount_from_string(amount3_str);

  env.generate_str_event("amount1 as string: " + amount1_str);
  env.generate_str_event("amount2 as string: " + amount2_str);

  // 200
  env.generate_str_event("test add_native_amounts");
  ...
  env.generate_str_event(
    "amount4 (amount1 + amount2) as string: " + amount4_str
  );

  // 50
  env.generate_str_event("test sub_native_amounts");
  const amount5 = env.sub_native_amounts(amount1, amount3);
  const amount5_str = env.native_amount_to_string(amount5);
  env.generate_str_event(
    "amount5 (amount1 - amount3) as string: " + amount5_str
  );

  // 200
  env.generate_str_event("test mul_native_amount");
  const amount6 = env.mul_native_amount(amount1, 2);
  const amount6_str = env.native_amount_to_string(amount6);
  env.generate_str_event("amount6 (amount1 * 2) as string: " + amount6_str);

  const amount7 = env.make_native_amount(152, 0);
  // [3, 2]
  env.generate_str_event("test div_rem_native_amount");
  ...
  env.generate_str_event(
    ...
    // [3, 2]
    env.generate_str_event("test div_rem_native_amounts");
